...
class HookAllInstancesTests: XCTestCase {
    ...
    func test_before() throws {
        ...
        XCTAssertFalse(run)
        try hookBefore(targetClass: MyObject.self, selector: #selector(MyObject.myMethod)) { [weak obj] in
            XCTAssertFalse(obj!.run)
            XCTAssertFalse(run)
            run = true
            XCTAssertFalse(obj!.run)
    ...
    func test_after() throws {
        ...
        XCTAssertFalse(obj.run)
        XCTAssertFalse(run)
        try hookAfter(targetClass: MyObject.self, selector: #selector(MyObject.myMethod(number:url:))) { [weak obj] in
            XCTAssertTrue(obj!.run)
            XCTAssertFalse(run)
            run = true
            XCTAssertTrue(obj!.run)
    ...
    func test_before_deinit() throws {
        ...
        try autoreleasepool {
            ...
            XCTAssertEqual(run, false)
            XCTAssertFalse(MyObject.isReleased)
            try hookDeallocBefore(targetClass: MyObject.self) {
                XCTAssertNil(reference)
                XCTAssertEqual(run, false)
                run = true
                XCTAssertNil(reference)
    ...
    func test_after_deinit() throws {
        ...
        try autoreleasepool {
            ...
            XCTAssertEqual(run, false)
            XCTAssertFalse(MyObject.isReleased)
            try hookDeallocAfter(targetClass: MyObject.self) {
                XCTAssertNil(reference)
                XCTAssertEqual(run, false)
                run = true
                XCTAssertNil(reference)
