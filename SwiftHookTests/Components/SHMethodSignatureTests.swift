...
class SHMethodSignatureTests: XCTestCase {
    ...
    func testNoDynamicMethod() {
        ...
        guard let signatureMethod = SHMethodSignature.init(objCTypes: method_getTypeEncoding(method)!) else {
            XCTFail()
            return
        }
        guard let signatureClosure = SHMethodSignature.init(objCTypes: sh_blockSignature({} as @convention(block) () -> Void)!) else {
            XCTFail()
            return
        }
    ...
    func testSimpleSignature() {
        ...
        guard let signatureClosure = SHMethodSignature.init(objCTypes: sh_blockSignature({_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _  in
    ...
    func testStructSignature() {
        ...
        guard let signatureClosure = SHMethodSignature.init(objCTypes: sh_blockSignature({_, _ in } as @convention(block) (CGPoint, CGRect) -> Void)!) else {
            XCTFail()
            return
        }
    ...
    func testArraySignature() {
        ...
        guard let signatureClosure = SHMethodSignature.init(objCTypes: sh_blockSignature({_, _, _ in } as @convention(block) ([Any], [Int], [CGRect]) -> Void)!) else {
            XCTFail()
            return
        }
        let argumentTypesExpect: [String] = ["@", "@", "@"]
    ...
    func testDictionarySignature() {
        ...
        guard let signatureClosure = SHMethodSignature.init(objCTypes: sh_blockSignature({_, _, _ in } as @convention(block) ([String: Any], [String: Int], [String: CGRect]) -> Void)!) else {
            XCTFail()
            return
        }
        let argumentTypesExpect: [String] = ["@", "@", "@"]
    ...
    func testClosureSignature() {
        ...
        guard let signatureClosure = SHMethodSignature.init(objCTypes: sh_blockSignature({_, _, _ in
            return {_, _ in
                return NSObject()
            }
